...
└── understudy/
    ...
    │   ├── watch.md                     open the live view; set up the status line
...
**Scores come from the personas' outcome, not from the finding count (v0.6.1).** Four lenses on one run wrote "every persona reached value and would come back" above 5/10 — a number produced by starting at the P1 ceiling (7) and subtracting per finding. Two causes, both fixed in the references rather than the ceilings: P1 was a prediction ("would realistically churn") with a round-up tie-break, so complaints became P1s; and the 0–10 bands had no anchor for what a 7 with a P1 looks like. Now P1 needs an observed moment and the P1/P2 tie-break rounds *down* with the risk named in *So what*; the bands carry a "looks like" column; the order of operations is personas first, ceiling second; and `check_report.py` prints a reconcile note (never a failure — the verdict is prose) when a positive verdict sits above a ≤5 score. The ceilings stay: they stop indefensible numbers, and they were never the problem.

**No face on the profile card unless asked (v0.7.4).** Three attempts — a drawn silhouette, a stock illustration, a trait-matched illustration — and a free image model that draws everyone at twenty-two; the Gemini free tier has zero image quota in the user's region and the user does not want to pay. The decision: `UNDERSTUDY_ICP_FACES` defaults to off and the card leads with the name and the line of life. The face pipeline stays for anyone with a key, behind the flag.

**A portrait must fit the person it stands for (v0.7.3).** A random illustrated face beside "Priya, 46, travel manager" reads as wrong, and the user said so. Two answers, in order: with no key, `icp_profiles.traits()` reads the Meet line — pronouns, age, work — and asks the illustration service for a matching head, hair colour, glasses and clothing, seeded so it is stable; with a key, `fetch_portrait()` generates a picture of the described person (Gemini free tier, Hugging Face free tier, or OpenAI), repeating the age in the prompt because models drift young. The free keyless text-to-image service was tried and rejected: it rate-limits and draws everyone at twenty-two; it stays as an explicit opt-in only. Never pay by default — the user does not want to.

**The face on a profile card is an illustration, fetched once and cached (v0.7.2).** The v0.7.1 silhouette-with-initials got the reaction it deserved ("what's this?"). `icp_profiles.fetch_avatar()` now pulls an Open Peeps portrait from DiceBear (seeded by the profile title, friendly expressions only, CC BY 4.0 credited in the section note), caches the SVG under `<run>/icp/avatars/`, and embeds it as a data URI in both renders. Offline on first render → the silhouette, still labelled. `UNDERSTUDY_AVATAR_STYLE` switches the DiceBear style. Nothing about the face is evidence; the note says so.

**ICP profiles are cards with a face, not bullets with thumbnails (v0.7.1).** `scripts/icp_profiles.py` parses `## ICP profiles` into structure and renders each profile as a card for both the PDF and the interactive report: an SVG avatar drawn from the profile's name (deterministic; no photo, no real person), the lens's `Meet:` line (a fictional person — first name, age, one clause — added to the contract in `agents/lens-icp.md`), the *Who*, then the details with fit and propensity as bars. Screenshot citations stay as plain text; the first version rendered them as thumbnails and the user's reaction was the right one — "we do not need them". A profile without a `Meet:` line gets a placeholder name from a fixed list and is labelled as such, so an old run still renders and a reader can tell the name was assigned.

**The HTML deliverable is an instrument, the PDF is a document (v0.7.0).** They read the same markdown but serve different rooms. `render_report.py --format pdf` prints the document layout through Chromium (the layout is also available as `--format print` for machines without it). `--format html` packs the run into `scripts/report_template.html` via `scripts/interactive_report.py`: one file, screenshots embedded downscaled, opens offline (the render itself fetches the site icon once, `UNDERSTUDY_SITE_LOGO=off` to skip), with a dashboard, a findings explorer, a per-persona session replay, the ICP profiles, a Present mode and browser-side triage with export. Rules the template keeps: nothing is scored or rewritten in JS — every number is read from the lens files; every quote and screenshot is the run's own; a missing field renders blank rather than failing; Pillow missing means full-size PNGs, not no images. The replay maps each log line to a screen by an explicit "Screenshot NN" mention, then a reaction logged at the same time, then whatever was showing before — good enough to follow, never claimed as exact.

**The renderer normalises what eight lenses write eight ways (v0.6.4).** `sentence()` sentence-cases the *Why that score* column; `where_cell()` turns the run summary's exact-but-unreadable *Where* citations (`persona-x/session.log:20 · 03-shot.png · Top 5 row 3`) into a file link, a thumbnail and an anchor; `site_logo()` fetches and caches the site icon for the cover. The markdown stays exact — the gate checks it — and the presentation is the renderer's job. First asked for after the trip.com run: "the information in Where is useless".

**A `both` run puts the visit and the journey for one persona in two folders (v0.6.3).** `persona-<slug>/` holds the website visit and `persona-<slug>-journey/` the product journey, both listed in the manifest's `personas` (the journey entry carries a `capture` note) and in `captures`. This is the convention the first *both* run settled on; nothing enforces it yet, and the cover page lists six "simulated users" for three personas as a result. Worth a renderer fix before the next *both* run.

**`check_report.py` decodes `findings-raw.json` before matching quotes (v0.6.3).** It read the file as text, and `json.dump` escapes `€` as `\u20ac`, so every quote carrying a price failed the verbatim check. The corpus now walks the decoded JSON's string values. Caught on the first run whose quotes had prices in them.

**The personas' words are evidence, and the gate reads them (v0.6.2).** The run summary carries an *In their own words* section — verbatim lines from session logs and debriefs beside the moment behind each top finding — because a quote is the one form of evidence an executive repeats in a meeting. That is also why it is the most dangerous section to write: a quote made more quotable is fabricated. So `check_report.py` normalises each row's quote (curly quotes, whitespace, ellipsis parts) and fails the evidence rule if it is not found verbatim in a persona file. Paraphrase belongs in findings; this section is a record.
...
5. **Seven lenses can produce a great deal of unread output.** The report template must force a one-sentence verdict and a top-3 before anything else. The reference skill did this, and it is why its exec summary was usable. **Enforced by gate check 6 in `check_report.py`** — this mitigation is a gate, not an intention.
   ...
   *Page numbers are the one thing not fixed: Chromium ignores CSS page-margin boxes and its built-in footer stamps the source `file://` path, which §7 forbids. Numbered sections and findings serve the same purpose and survive a change of paper size.*
   ...
   ✅ **Reshaped again 2026-09-16, after a three-lens product run was judged unshareable.** 45 findings for 26 problems, and the summary PDF showed the worst of them up to six times: in the Top 5, three times in "Raised by more than one check", and once per lens section. Three causes, all in `render_report.py`, none in the lenses:

   - **⚑ Corroboration pairs on evidence, never on product vocabulary.** The matcher paired any two findings quoting the same 12-character string, and on an AI product that is every finding that transcribes a button label — 34 of 45 were marked "also raised by", most falsely. It now pairs on the same *persona utterance* quoted under Evidence, the same screenshots cited (half of both sets in common, plus a second signal so a busy screen showing two problems does not pair them), or a title similarity ≥0.60. Thresholds were set against that run: 13 clusters, 26 distinct problems against 25 counted by hand.
   - **"Raised by more than one check" is one row per problem, never per finding.** Pairs close into clusters; the highest-severity member speaks for the cluster and every lens's own severity is printed beside its name (`Raised by Defects (P0) · Usability (P1) · Content (P1)`). Severities are still never averaged, and nothing is merged in the lens files — the cluster exists in the export only. The cover now carries the distinct-problem count next to the finding count.
...
10. **⚑ The user sees nothing while a run works.** Claude Code shows the orchestrator's text and a subagent panel; the hour-long traversal, where the persona thinks aloud into `session.log`, is invisible until it ends. Settled 2026-09-16 (v0.5.0): **the run folder is the display.** `watch.py` is a stdlib curses view for a second pane that reads only the run folder — persona progress against the cap, the auth pause (read from the log, so it cannot disagree with the traversal), screenshots as they land, the persona's commentary as it is written, lenses as they launch and finish. `mark.py` lets the orchestrator name the phase and the lenses it launched; everything else is derived, so the view cannot claim a state the disk does not show. Also `--line` for a status line and `--replay` for finished runs. **Revised the same day (v0.5.1): nobody types a path and nobody pastes JSON.** `init_run.py` records the newest run in `~/.understudy/last-run`, so `watch.py` needs no argument; the run command asks "open the live view?" and `watch.py --open` opens a terminal window (Ghostty, iTerm, Terminal.app, or prints the command); `/understudy:watch` does the same later and, on a yes, `watch.py --setup-statusline` writes the one settings entry a plugin cannot ship, keeping the rest of the file and leaving a copy. What was ruled out: hooks cannot print into the conversation, and auto-opening through a hook has a lifecycle nobody owns — a question at run start is the honest version. **Amended 2026-09-18 (v0.5.3): the status line clears itself.** `--line` printed "complete" for as long as the finished run stayed the newest one — days, at the bottom of every session. Past `COMPLETE_GRACE` (one hour after the manifest's `finished_utc`, or the manifest's mtime for older runs) it prints nothing, which Claude Code renders as no line. A finished run is old news; the run folder is still the record.
